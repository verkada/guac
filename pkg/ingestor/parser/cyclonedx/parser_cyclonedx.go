	...
	for _, vulnerability := range *c.cdxBom.Vulnerabilities {
		...
		if vulnerability.Analysis != nil {
			...
			if vulnerability.Analysis.State != "" {
				...
				// If state is empty, use UNDER_INVESTIGATION to indicate analysis pending
				// The affects array may indicate the component is affected, but VEX analysis hasn't been performed
				vd.Status = model.VexStatusUnderInvestigation
			}
